...
files_modified:
  ...
  - .github/workflows/staging-e2e-simulation.yml
autonomous: true
---
...
- [ ] `.github/workflows/staging-e2e-simulation.yml` updated with `simulation` job running `run-all.sh` (E2E-001 CI gate)
...
<action>
Modify `.github/workflows/staging-e2e-simulation.yml` to add a `simulation` job that runs the incident simulation suite. This satisfies E2E-001 ("CI gate blocks on failure").
...
<acceptance_criteria>
- `grep "run-all.sh" .github/workflows/staging-e2e-simulation.yml` returns a match
- `.github/workflows/staging-e2e-simulation.yml` contains a `simulation` job with `needs: [e2e]`
- `.github/workflows/staging-e2e-simulation.yml` simulation job contains `API_GATEWAY_URL` env var
- `.github/workflows/staging-e2e-simulation.yml` simulation job runs `bash scripts/simulate-incidents/run-all.sh`
