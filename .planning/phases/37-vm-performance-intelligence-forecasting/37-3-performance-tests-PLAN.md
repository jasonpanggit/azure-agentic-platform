...
files_modified: []
files_created:
  - agents/tests/compute/test_compute_performance.py
autonomous: true
---

# Plan 37-3: Unit Tests — Performance Intelligence Tools

Create `agents/tests/compute/test_compute_performance.py` with 15+ tests
across 3 test classes. Follow the `test_compute_guest_diagnostics.py` pattern
exactly: `@patch` decorators (outermost to innermost), `_instrument_mock()`
helper, `from agents.compute.tools import <func>` inside each test.

---

## Task 37-3-A: Create test file with shared helper and TestGetVmForecast (5 tests)

<read_first>
- agents/tests/compute/test_compute_guest_diagnostics.py (lines 1–236 — _instrument_mock helper + full TestExecuteRunCommand as the pattern to follow exactly)
- agents/compute/tools.py — get_vm_forecast implementation from Plan 37-1 (confirm patch targets: `agents.compute.tools.CosmosClient`, `agents.compute.tools.ForecasterClient`, `agents.compute.tools.get_credential`, `agents.compute.tools.instrument_tool_call`, `agents.compute.tools.get_agent_identity`)
- services/api-gateway/forecaster.py (lines 425–448 — get_forecasts return shape)
</read_first>

<action>
Create `agents/tests/compute/test_compute_performance.py` with the following content.
Every test patches at the `agents.compute.tools.*` path (not the source module path).
...
<acceptance_criteria>
- `wc -l agents/tests/compute/test_compute_performance.py` shows 300+ lines
- `grep -c "def test_" agents/tests/compute/test_compute_performance.py` outputs `15`
- `grep -n "class TestGetVmForecast" agents/tests/compute/test_compute_performance.py` returns a match
- `grep -n "class TestQueryVmPerformanceBaseline" agents/tests/compute/test_compute_performance.py` returns a match
- `grep -n "class TestDetectPerformanceDrift" agents/tests/compute/test_compute_performance.py` returns a match
- `grep -c "def _instrument_mock" agents/tests/compute/test_compute_performance.py` outputs `1`
- `grep -n "imminent_breach" agents/tests/compute/test_compute_performance.py` returns matches in at least 2 tests
- `grep -n "drift_score" agents/tests/compute/test_compute_performance.py` returns matches across TestDetectPerformanceDrift
- `grep -n "used_fallback_table" agents/tests/compute/test_compute_performance.py` returns a match in fallback test
- `python -m pytest agents/tests/compute/test_compute_performance.py --collect-only 2>&1 | grep "test session"` succeeds (collection works)
</acceptance_criteria>
...
```bash
# Confirm test file created and has correct counts
grep -c "def test_" agents/tests/compute/test_compute_performance.py
# Expected: 15

# Confirm three test classes
grep -c "^class Test" agents/tests/compute/test_compute_performance.py
# Expected: 3

# Run tests (requires agents/compute installed)
python -m pytest agents/tests/compute/test_compute_performance.py -v 2>&1 | tail -20
```

## must_haves

- File `agents/tests/compute/test_compute_performance.py` exists
- Exactly 15 test methods (`def test_` prefix)
- 3 test classes: `TestGetVmForecast`, `TestQueryVmPerformanceBaseline`, `TestDetectPerformanceDrift`
- Each class has exactly 5 test methods
- `_instrument_mock()` helper defined at module level
- All patches target `agents.compute.tools.*` path
- `test_drift_baseline_zero_guard` verifies no `ZeroDivisionError`
- `test_get_vm_forecast_imminent_breach_all_metrics` verifies `imminent_breach_count == 3`
- `test_baseline_insights_metrics_fallback` asserts `used_fallback_table is True` and `query_workspace.call_count == 2`
