# Plan 26-2: Forecaster Service

**Phase:** 26 — Predictive Operations
**Wave:** 2 (depends on 26-1: `baselines` Cosmos container must exist)
**Autonomous:** true
**Requirement:** INTEL-005 — pure-Python Holt double exponential smoothing, per-resource baselines, 15-min background sweep

---

## Goal

Create `services/api-gateway/forecaster.py` — the core forecasting engine for Phase 26. Add `MetricForecast` and `ForecastResult` Pydantic models to `models.py`. Write 15+ unit tests in `test_forecaster.py`.

No numpy, no statsmodels, no scipy. Pure Python arithmetic throughout.
...
| File | Change |
|---|---|
| `services/api-gateway/forecaster.py` | **Create** — full forecaster implementation |
| `services/api-gateway/models.py` | **Modify** — append `MetricForecast` and `ForecastResult` |
| `services/api-gateway/tests/test_forecaster.py` | **Create** — 15+ unit tests |

---

## Implementation

### 1. `services/api-gateway/forecaster.py`

Complete file structure (implement in this order):

#### 1a. Module header + imports
...
#### 1c. `FORECAST_METRICS` dictionary
...
#### 1d. Internal `_ForecastBaseline` dataclass
...
#### 1f. `_domain_for_resource_type` helper
...
#### 1g. `_emit_forecast_alert` — creates synthetic FORECAST_ALERT incident
...
### 2. `services/api-gateway/models.py`

Append two new models after `SLOCreateRequest` (current last model):
...
### 3. `services/api-gateway/tests/test_forecaster.py`

Write 15+ tests covering all pure functions and key `ForecasterClient` behaviors. Use `pytest` with `unittest.mock`. No real Cosmos or Azure Monitor calls.

#### Test structure (15 required tests):

**Pure function tests (8):**

1. `test_holt_smooth_empty_list` — `_holt_smooth([])` returns `(0.0, 0.0)`
2. `test_holt_smooth_single_value` — single element returns `(value, 0.0)`
3. `test_holt_smooth_flat_series` — constant series has near-zero trend
4. `test_holt_smooth_rising_series` — `[10, 20, 30, 40, 50]` → level ≈ 50, trend > 0
5. `test_holt_smooth_falling_series` — `[50, 40, 30, 20, 10]` → trend < 0
6. `test_compute_mape_perfect` — actual == predicted → MAPE = 0.0
7. `test_compute_mape_known_error` — 10% error series → MAPE ≈ 10.0
8. `test_compute_mape_skips_zero_actuals` — zeros in actual are skipped gracefully

**Time-to-breach tests (4):**

9. `test_ttb_rising_normal` — rising trend + headroom → positive minutes
10. `test_ttb_flat_trend_returns_none` — trend = 0 → None
11. `test_ttb_declining_normal_returns_none` — falling trend + normal metric → None
12. `test_ttb_invert_metric_falling` — falling trend + invert=True → positive minutes

**ForecasterClient tests (3+):**

13. `test_update_baseline_insufficient_data` — fewer than 2 points → returns None, no Cosmos write
14. `test_update_baseline_upserts_to_cosmos` — 24 points → Cosmos `upsert_item` called once with correct doc structure
15. `test_update_baseline_cosmos_error_non_fatal` — Cosmos raises → returns baseline (not None), logs warning
16. `test_get_forecasts_returns_empty_when_cosmos_none` — `ForecasterClient(None, None).get_forecasts(...)` → `[]`
17. `test_emit_forecast_alert_structure` — verify all required fields in returned dict

#### Test file header and sample test (implement all 17):
...
cd /path/to/repo
python -m pytest services/api-gateway/tests/test_forecaster.py -v

# 2. Verify no import errors in forecaster.py
python -c "from services.api_gateway.forecaster import ForecasterClient, _holt_smooth"

# 3. Verify models imported cleanly
python -c "from services.api_gateway.models import MetricForecast, ForecastResult"

# 4. Run all existing tests to check no regressions
python -m pytest services/api-gateway/tests/ -v --tb=short

# 5. Quick smoke test of Holt smoothing math
python -c "
from services.api_gateway.forecaster import _holt_smooth, _compute_time_to_breach
level, trend = _holt_smooth([10, 20, 30, 40, 50, 60, 70, 80])
print(f'level={level:.2f}, trend={trend:.2f}')
ttb = _compute_time_to_breach(level, trend, 90.0)
print(f'time_to_breach={ttb}m')
"
```
...
- [ ] `forecaster.py` created with all functions/classes listed above
- [ ] `_holt_smooth` returns correct `(level, trend)` tuple for rising series
- [ ] `_compute_time_to_breach` returns `None` for flat/declining trend
- [ ] `_compute_time_to_breach` returns positive minutes for rising trend with headroom
- [ ] `ForecasterClient.update_baseline` upserts correct doc structure to Cosmos
- [ ] `ForecasterClient.update_baseline` returns `None` (not raises) for <2 data points
- [ ] Cosmos write failure in `update_baseline` is non-fatal (returns baseline, logs warning)
- [ ] `run_forecast_sweep_loop` handles `CancelledError` by re-raising (clean shutdown)
- [ ] `FORECAST_ENABLED=false` causes sweep loop to exit without iteration
- [ ] `MetricForecast` and `ForecastResult` added to `models.py`
- [ ] All 17 tests pass in `test_forecaster.py`
...
- **No numpy/statsmodels/scipy**: The entire algorithm is implemented using built-in Python arithmetic. Keep it that way — container image size is a concern.
- **Thread safety**: `ForecasterClient` is not async-native. All Cosmos and Azure Monitor calls run via `asyncio.get_running_loop().run_in_executor(None, ...)` in the sweep loop. The `_get_container()` lazy init is safe because the sweep runs single-threaded in the executor.
- **Partial evidence is better than none**: Every step in `update_baseline` and `collect_metrics` handles errors individually. A resource that fails metric collection is silently skipped; it does not abort the sweep for other resources.
- **MAPE hold-out requires 24 points**: When fewer than 24 points are available (e.g., resource just added), `mape=0.0` and `confidence` is left as `"low"` — the forecast still runs but with declared low confidence.
- **`_post_forecast_alert` writes directly to Cosmos** rather than calling `POST /api/v1/incidents` via HTTP self-call. This avoids auth token management in the background thread and matches the pattern of other internal Cosmos writes in the codebase.
