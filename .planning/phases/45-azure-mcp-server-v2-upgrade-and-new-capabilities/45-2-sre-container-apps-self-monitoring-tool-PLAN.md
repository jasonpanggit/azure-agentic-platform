...
# Plan 45-2: SRE Container Apps Self-Monitoring Tool
...
The MCP `containerapps` namespace was already added to `ALLOWED_MCP_TOOLS` in Plan 45-1, but it only provides basic listing. The SDK tool provides deep revision and replica detail required for self-monitoring.
...
1. Add lazy import block after the existing `AzureChangeAnalysisManagementClient` import:
...
Add the following `@ai_function` tool function at the end of `agents/sre/tools.py`. The `start_time = time.monotonic()` line must be placed as the first line INSIDE the `with instrument_tool_call(...)` block — this matches the actual codebase pattern:
...
- `agents/sre/tools.py` `query_container_app_health` uses `with instrument_tool_call(` as a context manager
...
1. Add `query_container_app_health` to the import from `sre.tools`

2. Add `query_container_app_health` to the `tools=[...]` list in `create_sre_agent()`

3. Add `query_container_app_health` to the `tools=[...]` list in `create_sre_agent_version()` if it exists

4. Add `"query_container_app_health"` to the allowed_tools string list at the bottom of the system prompt

5. Add a "Platform Self-Monitoring" section to the SRE_AGENT_SYSTEM_PROMPT:
...
- `agents/sre/agent.py` `SRE_AGENT_SYSTEM_PROMPT` contains `containerapps` MCP tool reference
</acceptance_criteria>
</task>
...
<action>
Add the following test class to `agents/tests/sre/test_sre_tools.py`, after the existing test classes:
...
- `agents/tests/sre/test_sre_tools.py` contains 4 test methods in `TestQueryContainerAppHealth`
...
2. All `TestAllowedMcpTools` tests pass (including the updated count/entries from Plan 45-1)
...
<action>
1. In `agents/sre/tools.py`, find the `query_advisor_recommendations` function docstring. Update the `category` parameter documentation to explicitly list `OperationalExcellence` as a valid value:
...
2. In `agents/sre/agent.py`, in the `SRE_AGENT_SYSTEM_PROMPT`, add a note that the `OperationalExcellence` category is available:
...
- No functional code changes to `query_advisor_recommendations`
</acceptance_criteria>
</task>
...
assert 'with instrument_tool_call(' in source
