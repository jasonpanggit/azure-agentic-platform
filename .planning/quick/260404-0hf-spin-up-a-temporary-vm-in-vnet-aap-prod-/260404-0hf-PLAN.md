# Quick Plan: Spin Up Temporary VM for Runbook Seeding

**ID:** 260404-0hf
**Type:** quick (operator infrastructure task)
**Status:** COMPLETE
**Created:** 2026-04-04

## Goal

Spin up a temporary Ubuntu VM in `vnet-aap-prod/snet-container-apps`, run `seed.py` and `validate.py` against `aap-postgres-prod` (which is VNet-injected and unreachable from localhost), then delete the VM and all orphaned resources.

## Why a VM?

The existing `scripts/ops/19-4-seed-runbooks.sh` uses a temporary firewall rule to allow localhost access to the VNet-injected PostgreSQL server. However, PostgreSQL Flexible Server with VNet injection does **not** support firewall rules (only public-access mode does). A VM inside the VNet can reach PostgreSQL natively via the delegated subnet DNS.

## Context

- **VNet:** `vnet-aap-prod` in `rg-aap-prod` (East US 2)
- **Subnet:** `snet-container-apps`
- **PostgreSQL:** `aap-postgres-prod.postgres.database.azure.com` (VNet-injected, port 5432)
- **Credentials:** `terraform/envs/prod/credentials.tfvars` (gitignored)
- **Seed scripts:** `scripts/seed-runbooks/seed.py`, `validate.py`, `requirements.txt`
- **Runbook files:** `scripts/seed-runbooks/runbooks/*.md` (60 files)

## Pre-execution Checklist

- [ ] `az login` completed with Contributor on `rg-aap-prod`
- [ ] `terraform/envs/prod/credentials.tfvars` is readable locally
- [ ] Azure OpenAI endpoint + API key retrievable from Container App env vars

---

## Task 1: Create and Execute the Seed VM Script

Create a self-contained shell script `scripts/ops/seed-via-vm.sh` that does everything: provisions a VM, uploads scripts, runs seed + validate, and tears down.

### Script: `scripts/ops/seed-via-vm.sh`
...
- [ ] Script creates VM in `vnet-aap-prod/snet-container-apps` with `Standard_B1s`, no public IP
- [ ] Credentials extracted from `credentials.tfvars` (POSTGRES_PASSWORD) and Container App env (AZURE_OPENAI_*)
- [ ] `seed.py` runs successfully on VM, producing 60 runbooks in `runbooks` table
- [ ] `validate.py` runs successfully with all domains passing similarity threshold
- [ ] VM, NIC, and OS disk are deleted on exit (success or failure) via trap cleanup
- [ ] No secrets are hardcoded in the script itself
...
- [ ] Script completes end-to-end
- [ ] `runbooks` table contains 60 rows
- [ ] `validate.py` reports all domains PASS
- [ ] No orphaned VM/NIC/disk left in `rg-aap-prod`
- [ ] F-02 (BUG-002) resolved: `GET /api/v1/runbooks/search?q=vm+high+cpu&domain=compute` returns 200

---

## Task 3: Verify and Document

- [ ] Confirm no orphaned resources: `az vm list -g rg-aap-prod -o table` shows no `vm-seed-runbooks-*`
- [ ] Confirm disk cleanup: `az disk list -g rg-aap-prod -o table` shows no `vm-seed-runbooks-*` disks
- [ ] Update `.planning/STATE.md` to note F-02 is resolved (runbooks seeded)
...
| Risk | Mitigation |
|------|------------|
| VM creation fails (subnet delegation conflict) | `snet-container-apps` has Container App delegation; if VM placement fails, use `snet-reserved-1` (10.0.64.0/24) as fallback |
| `az vm run-command` timeout (90 min max) | seed.py processes ~60 runbooks sequentially; each takes ~10s for embedding = ~10min total; well within limit |
| Tarball too large for `run-command` | Chunked transfer implemented (50KB chunks) |
| Cleanup fails | Script prints manual cleanup commands; operator can run `az vm delete` + `az disk delete` + `az network nic delete` |
| AZURE_OPENAI_API_KEY not in Container App env | Fallback to Key Vault; if both fail, seed.py falls back to DefaultAzureCredential (managed identity) |
| Subnet NSG blocks outbound 443 | Per constraints, NSG on `snet-container-apps` allows outbound 443 |
