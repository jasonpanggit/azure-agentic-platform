...
const log = logger.child({ route: '/api/proxy/quota/scan' });

export const runtime = 'nodejs';
export const dynamic = 'force-dynamic';

/**
 * POST /api/proxy/quota/scan
 * Triggers a live scan of Compute quota usage across all subscriptions.
 */
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const res = await fetch(`${apiGatewayUrl}/api/v1/quota/scan`, {
      method: 'POST',
