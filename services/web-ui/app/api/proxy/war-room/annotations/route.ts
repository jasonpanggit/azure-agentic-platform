...
const log = logger.child({ route: '/api/proxy/war-room/annotations' });

export const runtime = 'nodejs';
export const dynamic = 'force-dynamic';

/**
 * POST /api/proxy/war-room/annotations?incident_id=<id>
 * Body: { content, trace_event_id? }
 *
 * Proxies to POST /api/v1/incidents/{id}/war-room/annotations
 */
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const res = await fetch(
      `${apiGatewayUrl}/api/v1/incidents/${encodeURIComponent(incidentId)}/war-room/annotations`,
      {
        method: 'POST',
