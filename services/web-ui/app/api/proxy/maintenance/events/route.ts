...
const log = logger.child({ route: '/api/proxy/maintenance/events' });

export const runtime = 'nodejs';
export const dynamic = 'force-dynamic';

/**
 * GET /api/proxy/maintenance/events
 * Proxies to GET /api/v1/maintenance/events with optional query params.
 */
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const upstream = `${apiGatewayUrl}/api/v1/maintenance/events${qs ? `?${qs}` : ''}`;
