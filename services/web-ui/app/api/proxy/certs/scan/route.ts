...
const log = logger.child({ route: '/api/proxy/certs/scan' });

export const runtime = 'nodejs';
export const dynamic = 'force-dynamic';

/**
 * POST /api/proxy/certs/scan
 * Triggers an ARG scan for expiring TLS certificates.
 */
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const res = await fetch(`${apiGatewayUrl}/api/v1/certs/scan`, {
      method: 'POST',
