...
const log = logger.child({ route: '/api/proxy/budgets/scan' });

export const runtime = 'nodejs';
export const dynamic = 'force-dynamic';

/**
 * POST /api/proxy/budgets/scan
 * Triggers a live scan of subscription budgets and spending.
 */
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const res = await fetch(`${apiGatewayUrl}/api/v1/budgets/scan`, {
      method: 'POST',
