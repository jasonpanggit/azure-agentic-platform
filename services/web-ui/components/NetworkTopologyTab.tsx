...
// Short type abbreviations shown as crisp text badges on each node.
// SVG <text> is vector-rendered — stays sharp at any zoom level, unlike
// rasterized path icons which blur when Cytoscape zooms in.
...
// Generate a pill-shaped SVG text badge.  The viewBox is small (52×20) but
// the raster size is 8× larger so Cytoscape downscales a crisp high-DPI bitmap.
const typeBadge = (abbr: string, color: string): string => {
  const w = 52, h = 20, r = 3
  const svgStr = [
    `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${w} ${h}" width="${w * 8}" height="${h * 8}">`,
    `<rect x="0" y="0" width="${w}" height="${h}" rx="${r}" fill="${color}" opacity="0.9"/>`,
    `<text x="${w / 2}" y="${Math.round(h * 0.72)}" `,
    `font-family="Inter,ui-monospace,monospace" font-size="10.5" font-weight="700" `,
    `fill="white" text-anchor="middle">${abbr}</text>`,
    `</svg>`,
  ].join('')
  return `data:image/svg+xml,${encodeURIComponent(svgStr)}`
}

// Pre-build badge data URIs for every resource type
const TYPE_BADGES: Record<string, string> = Object.fromEntries(
  Object.entries(TYPE_ABBR).map(([type, abbr]) => [
    type,
    typeBadge(abbr, TYPE_COLOR[type] ?? '#475569'),
  ])
)
...
const cytoscapeStylesheet: CytoscapeStylesheet[] = [
  // ── Base node ─────────────────────────────────────────────────────────────
  {
    ...
    style: {
      ...
      'font-size': '11px',
      'font-weight': '500',
      // Label sits to the right of the type badge (badge occupies left ~58px)
      'text-valign': 'center',
      'text-halign': 'right',
      'text-wrap': 'ellipsis',
      ...
      width: NODE_W,
      height: NODE_H,
      // Type badge sits in the left portion of the card
  ...
  // ── Per-type: accent border + type badge ─────────────────────────────────
  ...Object.entries(TYPE_COLOR).map(([type, color]) => ({
    selector: `node[type="${type}"]`,
    style: {
      'border-color': color,
      'border-width': 2,
      'background-image': TYPE_BADGES[type] ?? 'none',
    } as Record<string, unknown>,
  })),
  // VNet is slightly larger and bolder
  { selector: 'node[type="vnet"]',    style: { 'font-weight': '600', width: '190px', height: '52px' } },
  { selector: 'node[type="subnet"]',  style: { 'background-color': '#141d2e', 'border-width': 1, 'border-style': 'dashed' } },
  { selector: 'node[type="external"]',style: { 'background-color': '#111827', 'border-style': 'dashed', color: '#6b7280', 'background-image': TYPE_BADGES.external } as Record<string, unknown> },
  // ── Health overlays ───────────────────────────────────────────────────────
