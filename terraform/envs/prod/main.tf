...
module "keyvault" {
  ...
  # Allow Container Apps subnet to reach KV via VNet service endpoint.
  # The subnet has Microsoft.KeyVault service endpoint enabled so traffic routes
  # directly through the VNet backbone rather than the public internet.
  allowed_subnet_ids = [module.networking.subnet_container_apps_id]
